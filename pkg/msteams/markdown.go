...
var (
	...
	// stickerImgRegex catches Teams Stickers, Giphy and GenericFile images that
	// embed a direct src URL rather than an AMS object reference.
	stickerImgRegex = regexp.MustCompile(`(?is)<img[^>]+itemtype=["']http://schema\.skype\.com/(?:Sticker|Giphy|Emoji|EmojiAsImage|FlikMsgPreview|VideoMsg)["'][^>]*>`)
...
// ExtractAMSAttachments pulls every image/file embedded in a Teams chat
// body: AMS-hosted pictures + files, plus Teams Stickers/Giphy/emoji tags
// whose src is a direct CDN URL. Regex-based because the chat service
// serialises tags inconsistently and an HTML parser fights us on edge cases.
